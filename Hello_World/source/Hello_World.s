@ ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
@ Header and stack are automatically handled by devkitARM's crt0.s boot stub!
@ We just need to start at "main"
...
main:
	@ devkitARM already initializes the stack, so no need for mov sp,#0x03000000
	
	mov r4, #0x04000000  		@ DISPCNT - LCD Control base address
	mov r2, #0x403    			@ 4 = Layer 2 on / 3 = ScreenMode 3
	str	r2, [r4]         	

	ldr r1, =HelloWorld			@ Load Address of Hello World Message
...
PrintChar:
	...
	mov r6, #16				    @ Xpos, 2 bytes per pixel, 8 bytes per char (8*2)
	mul r2, r4, r6
	add r3, r3, r2
	
	ldr r4, =3840			    @ Ypos, 240 pixels per line, 2 bytes per pixel, 8 lines per char
	mul r2, r5, r4				
	add r3, r3, r2
	...
	add r4, r4, r0, lsl #3		@ 8 bytes per char (GNU uses lsl instead of asl)
	
	mov r1, #8				    @ 8 lines 
DrawLine:
	mov r7, #8 				    @ 8 pixels per line
	ldrb r8, [r4], #1			@ Load Letter
	mov r9, #128		        @ Mask (0b10000000)
			
	ldr r2, =0xFF40             @ Color: ABBBBBGGGGGRRRRR   A=Alpha (0b1111111101000000)
DrawPixel:
	tst r8, r9				    @ Is bit 1?
	strhne r2, [r3]			    @ Yes? then fill pixel (HalfWord). GNU uses strhne instead of strneh
	add r3, r3, #2
	mov r9, r9, ror #1		    @ Bitshift Mask
	subs r7, r7, #1
	bne DrawPixel			    @ Next Hpixel
	
	add r3, r3, #464	        @ Move Down a line (480-16) 
